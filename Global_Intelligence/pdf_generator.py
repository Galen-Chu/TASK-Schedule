...
# ---- Editorial content: (domain_zh, domain_en, category, brand ramp, rows) --
# ramp = [tint, light, base, dark, darkest]; base is the domain accent.
# rows = [org, focus, time, analysis]
PAGES = [
    ("地緣政治與國際關係", "Geopolitics & International Relations", "Category 01", T.RAMP_INK, [
    ...
    ("巨觀經濟與金融市場", "Macroeconomics & Financial Markets", "Category 02", T.RAMP_AMBER, [
        ["歐洲央行 (ECB)", "最新貨幣通報與通膨警告", "2026-08-06 10:00 CEST", "ECB 指出歐元區頭條通膨雖受控制，但能源波動仍存。主���央行延後大幅降息，高利率 Higher for Longer 成為常態。"],
    ...
    ("資訊科技與人工智慧", "IT, AI & Semiconductors", "Category 03", T.RAMP_TEAL, [
    ...
    ("生物科技與健康醫療", "Biotech & Healthcare", "Category 04", T.RAMP_SAGE, [
    ...
    ("硬體工程、自動化與能源轉型", "Hardware, Automation & Energy", "Category 05", T.RAMP_CORAL, [
...
def build_global_pdf(filename, data=None, date_str=None):
    ...
    for idx, (domain_zh, domain_en, category, ramp, rows) in enumerate(PAGES):
        if idx > 0 or digest:
